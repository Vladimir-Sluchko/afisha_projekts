...
import by.itacademy.afisha.service.dto.error.FieldErrorForMultiple;
import by.itacademy.afisha.service.dto.error.MultipleError;
import by.itacademy.afisha.service.dto.error.SingleError;
...
public class GlobalHandler {
    ...
    public List<MultipleError> handle(ConstraintViolationException e){
        ...
        List<FieldErrorForMultiple> fieldErrors = new ArrayList<>();
        Set<ConstraintViolation<?>> set = e.getConstraintViolations();
            for (ConstraintViolation<?> constraintViolation : set) {
                String s = constraintViolation.getPropertyPath().toString();
                fieldErrors.add(new FieldErrorForMultiple((FIELD + s.substring(s.lastIndexOf(".") + 1))
                        ,constraintViolation.getMessage()));
            }
            multipleErrors.add(new MultipleError(fieldErrors));
